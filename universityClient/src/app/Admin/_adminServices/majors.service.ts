...
export class MajorsService {
  ...
  addNewFaculty(major) {
    return this.http.post(this.baseUrl + 'majors/addmajor', major);
  }

  getFacultyMajors(facultyId) {
    return this.http.get<any[]>(
      this.baseUrl + 'majors/facultyMajors/' + facultyId
    );
  }

  getmajorsList(page?: number, itemsPerPage?: number, facultyId?: number) {
    let params = this.getPaginationHeaders(page, itemsPerPage, facultyId);
    return this.http
      .get<any[]>(this.baseUrl + 'majors/getMajorsList', {
        observe: 'response',
        params,
      })
      .pipe(
